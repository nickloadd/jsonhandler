/****************************************************************************
** Meta object code from reading C++ file 'tserver.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.9.2)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../src/tserver.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'tserver.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.9.2. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
QT_WARNING_PUSH
QT_WARNING_DISABLE_DEPRECATED
struct qt_meta_stringdata_TServer_t {
    QByteArrayData data[7];
    char stringdata0[43];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_TServer_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_TServer_t qt_meta_stringdata_TServer = {
    {
QT_MOC_LITERAL(0, 0, 7), // "TServer"
QT_MOC_LITERAL(1, 8, 9), // "servClose"
QT_MOC_LITERAL(2, 18, 0), // ""
QT_MOC_LITERAL(3, 19, 4), // "send"
QT_MOC_LITERAL(4, 24, 2), // "id"
QT_MOC_LITERAL(5, 27, 4), // "data"
QT_MOC_LITERAL(6, 32, 10) // "onRadyRead"

    },
    "TServer\0servClose\0\0send\0id\0data\0"
    "onRadyRead"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_TServer[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       4,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       3,       // signalCount

 // signals: name, argc, parameters, tag, flags
       1,    0,   34,    2, 0x06 /* Public */,
       3,    2,   35,    2, 0x06 /* Public */,
       3,    1,   40,    2, 0x06 /* Public */,

 // slots: name, argc, parameters, tag, flags
       6,    2,   43,    2, 0x0a /* Public */,

 // signals: parameters
    QMetaType::Void,
    QMetaType::Void, QMetaType::QString, QMetaType::QJsonDocument,    4,    5,
    QMetaType::Void, QMetaType::QJsonDocument,    5,

 // slots: parameters
    QMetaType::Void, QMetaType::QString, QMetaType::QJsonDocument,    4,    5,

       0        // eod
};

void TServer::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        TServer *_t = static_cast<TServer *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->servClose(); break;
        case 1: _t->send((*reinterpret_cast< QString(*)>(_a[1])),(*reinterpret_cast< QJsonDocument(*)>(_a[2]))); break;
        case 2: _t->send((*reinterpret_cast< QJsonDocument(*)>(_a[1]))); break;
        case 3: _t->onRadyRead((*reinterpret_cast< QString(*)>(_a[1])),(*reinterpret_cast< QJsonDocument(*)>(_a[2]))); break;
        default: ;
        }
    } else if (_c == QMetaObject::IndexOfMethod) {
        int *result = reinterpret_cast<int *>(_a[0]);
        void **func = reinterpret_cast<void **>(_a[1]);
        {
            typedef void (TServer::*_t)();
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&TServer::servClose)) {
                *result = 0;
                return;
            }
        }
        {
            typedef void (TServer::*_t)(QString , QJsonDocument );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&TServer::send)) {
                *result = 1;
                return;
            }
        }
        {
            typedef void (TServer::*_t)(QJsonDocument );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&TServer::send)) {
                *result = 2;
                return;
            }
        }
    }
}

const QMetaObject TServer::staticMetaObject = {
    { &QTcpServer::staticMetaObject, qt_meta_stringdata_TServer.data,
      qt_meta_data_TServer,  qt_static_metacall, nullptr, nullptr}
};


const QMetaObject *TServer::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *TServer::qt_metacast(const char *_clname)
{
    if (!_clname) return nullptr;
    if (!strcmp(_clname, qt_meta_stringdata_TServer.stringdata0))
        return static_cast<void*>(this);
    return QTcpServer::qt_metacast(_clname);
}

int TServer::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QTcpServer::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 4)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 4;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 4)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 4;
    }
    return _id;
}

// SIGNAL 0
void TServer::servClose()
{
    QMetaObject::activate(this, &staticMetaObject, 0, nullptr);
}

// SIGNAL 1
void TServer::send(QString _t1, QJsonDocument _t2)
{
    void *_a[] = { nullptr, const_cast<void*>(reinterpret_cast<const void*>(&_t1)), const_cast<void*>(reinterpret_cast<const void*>(&_t2)) };
    QMetaObject::activate(this, &staticMetaObject, 1, _a);
}

// SIGNAL 2
void TServer::send(QJsonDocument _t1)
{
    void *_a[] = { nullptr, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 2, _a);
}
QT_WARNING_POP
QT_END_MOC_NAMESPACE
